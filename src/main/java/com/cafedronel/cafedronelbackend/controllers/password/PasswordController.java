...
public class PasswordController {
    ...
    @PostMapping("/forgot")
    public ResponseEntity<MessageResponse<Boolean>> forgotPassword(@Valid @RequestBody ForgotRequest forgotRequest) {
        return ResponseEntity.ok(new MessageResponse<>(passwordService.forgotAccount(forgotRequest)));
    }


    @PostMapping("/reset-password")
    public ResponseEntity<MessageResponse<Boolean>> resetPassword(@Valid @RequestBody ResetPasswordRequest resetPasswordRequest) {
